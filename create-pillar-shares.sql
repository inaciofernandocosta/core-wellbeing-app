-- Tabela para gerenciar compartilhamentos de pilares
...
-- Proprietário pode ver todos os compartilhamentos que criou
CREATE POLICY "Users can view their own shares"
  ON pillar_shares
  FOR SELECT
  USING (auth.uid() = owner_id);

-- Proprietário pode criar compartilhamentos
...
-- Proprietário pode atualizar compartilhamentos
CREATE POLICY "Users can update their own shares"
  ON pillar_shares
  FOR UPDATE
  USING (auth.uid() = owner_id);

-- Proprietário pode deletar compartilhamentos
CREATE POLICY "Users can delete their own shares"
  ON pillar_shares
  FOR DELETE
  USING (auth.uid() = owner_id);

-- Usuários compartilhados podem ver os compartilhamentos deles
...
-- Atualizar políticas de pillars para permitir visualização compartilhada
...
-- Atualizar políticas de pillars para permitir edição compartilhada
